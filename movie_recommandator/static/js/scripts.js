...
async function fetchMovies() {
  ...
  try {
    ...
    // Handle failed request or unexpected content
    if (!res.ok) {
      const text = await res.text();
      console.error("❌ /api/search returned non-OK status", res.status, text);
      movieList.innerHTML = `<p>❌ Could not fetch movie data. (${res.status})</p>`;
      return;
    }
    ...
    if (!recRes.ok) {
      ...
      console.error("❌ /api/recommend returned non-OK status", recRes.status, text);
      recommendationList.innerHTML = `<p>❌ Could not fetch recommendations. (${recRes.status})</p>`;
      return;
    }
    ...
    movieList.innerHTML = `<p>❌ Could not fetch movie data. Please try again.</p>`;
  }
}
